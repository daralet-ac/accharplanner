...
import { MAX_SKILL_INVESTED_TRAINED, MAX_SKILL_INVESTED_SPECIALIZED } from "./constants";
...
export const importCharacter = function (store: Store<State>, json: any) {
  ...
  Object.keys(json.attribs).forEach((a) => {
    store.state.build.character.attributes[a].creation = json.attribs[a].creation;
    store.state.build.character.attributes[a].invested = json.attribs[a].base - json.attribs[a].creation;
  });

  // Vitals
  Object.keys(json.vitals).forEach((a) => {
    store.state.build.character.vitals[a].invested = 0;
    store.state.build.character.vitals[a].invested = json.vitals[a].base - store.getters[a + "Base"];
  });

  // Skills
  Object.keys(json.skills).forEach((s) => {
    store.state.build.character.skills[s].training = json.skills[s].training.toLowerCase();
    ...
    if (
      ...
      store.state.build.character.skills[s].invested = json.skills[s].base - store.getters[s + "Base"];
